...
@section('content')
    ...
    <hr>
    <div class="Outputs">
        {!! Form::open(["url"=>"create/proposal/FacultyMember"]) !!}
            <div class="form-group col-md-6">
                <label for="AcademicYear"> <i class="fa fa-caret-left"></i> العام الجامعي :</label>
                <input  title="العام الجامعي" type="text" class="form-control" readonly disabled
                value="@if(AcademicYear_GPC() == null)لايوجد بيانات ليتم عرضها !@endif 
                @if(AcademicYear_GPC() != null){{AcademicYear_GPC()}} @endif">
                </input>
            </div>
            <div class="form-group col-md-6">
                <label for="Semester"> الفصل الدراسي :</label>
                <input title="الفصل الدراسي" type="text" class="form-control" readonly disabled value="@if(Semester_GPC() == null)لايوجد بيانات ليتم عرضها !@endif
                @if(Semester_GPC() != null) {{ Semester_GPC()}} @endif"></input>
            </div>
            ...
            <div class="form-group col-md-12 ">
                <div class="ProgramChoose">
                    @foreach($Program as $p)
                        <div class="form-group col-md-6">
                            <ul>
                                <li><input type="checkbox" name="Program[]" value="{{$p->id}}"> {{$p->ProgramName}}</li>
                            </ul>
                        </div>
                    @endforeach
                </div>    
            ...
            <div class="form-group col-md-12 {{ $errors->has('PropertyRight') ? ' has-error' : '' }} ">
                <label for="PropertyRight"> <i class="fa fa-caret-left"></i> هل ترغب في الاشراف علي هذا المقترح ؟</label>
                {!! Form::select('PropertyRight',['3'=>'نعم ، أرغب في الاشراف عليه .','4'=>'لا ،أرغب في الاشراف عليه .'],null,array('required','id'=>'PropertyRight','class'=>'form-control','placeholder'=>'--- أختر أجابتك من هنا ---','title'=>'تحديد حقوق الملكية الفكرية لهذا المقترح')) !!}
            ...
            <div class="form-group col-lg-4">
                <button title="تقديم" type="submit" class="btn btn-primary form-control"> <strong>تقديم</strong> <i class="fa fa-arrow-circle-left"></i> </button>
            </div>
                <!-- div Reset butten -->
            <div class="form-group col-lg-2">
                <button title="ألغاء" type="reset" class="btn btn-danger form-control"> <strong>ألغاء</strong> <i class="fa fa-times-circle"></i> </button>
            </div>
        {!! Form::close() !!}
    </div>
@endsection    
